    ...
    <div class="card text-white md:w-[760px] w-[320px] bg-black/20 p-4 mb-4 rounded">
        <form method="POST" action="{{ url('pets') }}" class="flex flex-col md:flex-row gap-4 mt-4" enctype="multipart/form-data">
            ...
            <div class="w-full md:w-[320px]">
                ...
                <input type="number" step="0.1" class="input bg-[#0009] outline-0" name="weight" placeholder="12.5" value="{{ old('weight') }}">
                @error('weight')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Location --}}
                <label class="label text-white">Location:</label>
                <input type="text" class="input bg-[#0009] outline-0" name="location" placeholder="Colombia" value="{{ old('location') }}">
                @error('location')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
                {{-- Description --}}
                <label class="label text-white">Description:</label>
                <textarea class="textarea bg-[#0009] outline-0 w-full" name="description" placeholder="Describe the pet..." rows="3">{{ old('description') }}</textarea>
                @error('description')
                    <small class="badge badge-error w-full mt-1 text-xs py-4">{{ $message }}</small>
                @enderror
